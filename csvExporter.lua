...
coords = "%xpos%, %ypos%, %name%\n"

function generateSprites( )
	for i,v in ipairs(spriteLayers) do
		layer = spriteLayers[i][2]		
		--layerText = as3.tolua(DAME.CreateTextForSprites(layer, coords, "Avatar"))
		lText = as3.tolua(DAME.CreateTextForSprites(layer, coords, "Avatar"))
		layerText = string.gsub(lText, '"', '')
...
for groupIndex = 0,groupCount do
	...
	for layerIndex = 0,layerCount do
		...
		if isMap == true then
			mapFileName = groupName.."_"..layerName..".csv"
	...
	--	fileText = fileText.."<level name=\""..groupName.."\" minx=\""..minx.."\" miny=\""..miny.."\" maxx=\""..maxx.."\" maxy=\""..maxy.."\" bgColor = \""..as3.tolua(DAME.GetBackgroundColor()).."\" >\n"
		...
		--	fileText = fileText..layers[i][4]
			...
			if as3.tolua(layer.IsSpriteLayer()) == true then
	spriteFileName = groupName.."_"..layerName..".csv"
		DAME.WriteFile(projectDataDir.."/"..spriteFileName, fileText )
			--	fileText = fileText.."\n"..layerName.."\n"
			end
